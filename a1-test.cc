...
void test1() {
    ...
    //dbfile.createMetaData(rel->metadataPath(), heap, NULL);
    dbfile.Create(rel->path(), rel->metadataPath(), heap, NULL);

    char tbl_path[100]; // construct path of the tpch flat text file
    sprintf(tbl_path, "%s%s.tbl", tpch_dir, rel->name());
    cout << " tpch file will be loaded from " << tbl_path << endl;
    
    int readStatus = dbfile.Open(rel->path(), rel->metadataPath());
...
void test2() {
    ...
    DBFile dbfile;
    dbfile.Open(rel->path(), rel ->metadataPath());
    dbfile.MoveFirst();
