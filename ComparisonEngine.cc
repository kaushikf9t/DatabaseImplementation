...
int ComparisonEngine::buildQueryOrderMaker(OrderMaker *sortOrder, OrderMaker *newOrder, CNF *cnf) {
    for(int sortAtt = 0; sortAtt < sortOrder->numAtts; sortAtt++){
        for(int cnfAtt = 0; cnfAtt < cnf->numAnds; cnfAtt++){
            if(cnf->orLens[sortAtt] == 1){
                Comparison comp = cnf->orList[sortAtt][0];
                if(comp.op == Equals){
                    if((comp.whichAtt1 == sortOrder->whichAtts[sortAtt] && comp.operand2 == Literal) || (comp.whichAtt1 == Literal && comp.whichAtt2 == sortOrder->whichAtts[sortAtt])){
                        newOrder->whichAtts[newOrder->numAtts] = sortOrder->whichAtts[sortAtt];
                        newOrder->whichTypes[newOrder->numAtts] = sortOrder->whichTypes[sortAtt];
                        break;
                    } else {
                        continue;
                    }
